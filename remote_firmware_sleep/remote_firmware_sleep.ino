...
void setup() {
  ...
  pCmd_RHRD.addCommand((byte*) "\xFF\x51","SLEEP.SEC",     SLEEP_SEC_pCmd_action_handler);           // Turns LED off  ("
  ...
  pCmd_RHRD.addCommand((byte*) "\x50","SLEEP!",            NULL);
...
void SENSOR_READ_sCmd_query_handler(SerialCommand this_sCmd){
  int pin;
  uint16_t value;
  char *arg = this_sCmd.next();
  if (arg == NULL){
...
void SLEEP_SEC_pCmd_action_handler(PacketCommand& this_pCmd) {

  #if defined(DEBUG)
  DEBUG_PORT.println(F("# SLEEP_SEC_pCmd_action_handler"));
  #endif


  
  uint16_t seconds;
  this_pCmd.unpack_uint16(seconds);

     uint16_t repeats;
  this_pCmd.unpack_uint16(repeats);

  

  radiosleep();

  for (int i=0; i< (uint16_t) repeats; i++) {
    rtc.setAlarmSeconds((rtc.getSeconds() + (uint8_t) seconds) % 60); 
  rtc.enableAlarm(rtc.MATCH_SS);
  rtc.standbyMode(); // sleep uC for seconds
}

  // after this, we've awoken
